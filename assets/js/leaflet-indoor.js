...
L.Indoor = L.Layer.extend({
  ...
  addData: function (data, options) {
    ...
    features.forEach(function (part) {
      var level =
        options.level === undefined ? options.getLevel(part) : options.level;
      var layer;
        ...
        Object.keys(layer._layers).forEach(function (roomLayerId) {
          if (layer._layers[roomLayerId].feature.properties.name) {
            var myIcon = L.divIcon({
              ...
              html: layer._layers[roomLayerId].feature.properties.name,
            });
            var textPos = layer._layers[roomLayerId].getBounds().getCenter();
            L.marker(textPos, { icon: myIcon, interactive: false }).addTo(
              layer.roomLabels
            );
          }
        });
  ...
  toggleLabels: function () {
    if (
      Number.isInteger(this.options.minCaptionZoom) &&
      this._level !== undefined &&
      this._map.getZoom() >= this.options.minCaptionZoom
    ) {
      this._layers[this._level].addLayer(this._layers[this._level].roomLabels);
    } else if (this._level) {
      this._layers[this._level].removeLayer(
        this._layers[this._level].roomLabels
      );
    }
  },
  getLevel: function () {
  ...
  onAdd: function (map) {
    ...
    for (i = levels.length - 1; i >= 0; i--) {
      ...
      levelBtn.appendChild(
        levelBtn.ownerDocument.createTextNode(originalLevel)
      );
